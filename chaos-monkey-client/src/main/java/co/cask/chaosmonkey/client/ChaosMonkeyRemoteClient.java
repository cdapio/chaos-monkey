 ...
 */
public class ChaosMonkeyRemoteClient implements ChaosMonkeyClient {
  ...
  public ChaosMonkeyRemoteClient(String hostname, int port) {
    this(hostname, port, false);
  }

  public ChaosMonkeyRemoteClient(String hostname, int port, boolean sslEnabled) {
  ...
  public void rollingRestart(String service, int restartTimeSeconds, int delaySeconds)
    throws IOException {
    rollingRestartWithRequestBody(service, String.format("{restartTime:%d,delay:%d}", restartTimeSeconds,
                                                         delaySeconds));
  }

  /**
   * Starts a rolling restart on 'count' nodes chosen arbitrarily using given restart time and delay
   *
   * @param service The name of the service to perform rolling restart on
   * @param count The number of nodes affected
   * @param restartTimeSeconds Number of seconds a service is kept offline before restarting
   ...
   */
  @Override
  public void rollingRestart(String service, int count, int restartTimeSeconds, int delaySeconds) throws IOException {
    rollingRestartWithRequestBody(service, String.format("{count:%d,restartTime:%d,delay:%d}", count,
                                                         restartTimeSeconds, delaySeconds));
  }

  /**
   * Starts a rolling restart on percentage of nodes chosen arbitrarily using given restart time and delay
   *
   * @param service The name of the service to perform rolling restart on
   * @param percentage Number between 0 and 1 to represent the percent of nodes affected
   * @param restartTimeSeconds Number of seconds a service is kept offline before restarting
  ...
  public void rollingRestart(String service, double percentage, int restartTimeSeconds, int delaySeconds)
    throws IOException {
    rollingRestartWithRequestBody(service, String.format("{percentage:%f,restartTime:%d,delay:%d}", percentage,
                                                         restartTimeSeconds, delaySeconds));
  }

  /**
   * Starts a rolling restart on given nodes using given restart time and delay
   *
   * @param service The name of the service to perform rolling restart on
   * @param nodes Collection of ip addresses of affected nodes
   * @param restartTimeSeconds Number of seconds a service is kept offline before restarting
  ...
  public void rollingRestart(String service, Collection<String> nodes, int restartTimeSeconds, int delaySeconds)
    throws IOException {
    String collectionJSON = GSON.toJson(nodes);
    rollingRestartWithRequestBody(service, String.format("{nodes:%s,restartTime:%d,delay:%d}", collectionJSON,
                                                         restartTimeSeconds, delaySeconds));
  }
