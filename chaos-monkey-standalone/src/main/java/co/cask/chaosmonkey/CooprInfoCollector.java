...
public class CooprInfoCollector implements ClusterInfoCollector {
  ...
  @Override
  public Collection<NodeProperties> getNodeProperties(Configuration conf) throws Exception {
    String clusterId = conf.get(Constants.Coopr.CLUSTER_ID);
    if (clusterId == null || clusterId.isEmpty()) {
      throw new IllegalArgumentException("Cluster ID not specified");
    }

    URL url = new URL(conf.get(Constants.Coopr.SERVER_URI) + "/" + conf.get(Constants.Coopr.API_VERSION) + "/" +
                        "getNodeProperties");
    HttpRequest request = HttpRequest.get(url)
      .addHeader("coopr-userid", conf.get(Constants.Coopr.USER_ID))
      .addHeader("coopr-tenantid", conf.get(Constants.Coopr.TENANT_ID))
